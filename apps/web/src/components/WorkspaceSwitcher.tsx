 ...
 * that manages every workspace (open / wake / stop / create) via the host
 * control plane. Replaces the T3 project picker — a workspace switch is a
 * machine switch, so Open hard-navigates to `/w/:id/` for a clean remount.
 */
            ...
            <div className="max-h-80 overflow-y-auto">
              {(workspaces ?? []).map((workspace) => {
                ...
                return (
                  <div
                    key={workspace.id}
                    className={`flex min-w-0 items-center gap-2 rounded-md px-2 py-1.5 ${isCurrent ? "bg-accent/60" : "hover:bg-accent"}`}
                  >
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      {workspace.state === "running" ? (
                        <Button
                          ...
                          onClick={() => void runAction(workspace.id, "stop")}
                        >
                          Stop
                        </Button>
                      ) : (
                        <Button
                          ...
                          variant="ghost"
                          disabled={busyId === workspace.id}
                          onClick={() => void runAction(workspace.id, "wake")}
                        >
                          Wake
                        </Button>
                      )}
