...
You are an agent. Nero supports **agentic registration**: discover → register → (claim if needed) → exchange for an access_token → call API → handle revocation. Follow the steps in order; do not skip ahead.

This document is for agents. **Humans authenticate only through WorkOS AuthKit** (`GET https://grogan.dev/auth/login` or `GET https://nero.grogan.dev/auth/login`). Do not use this registration flow as a browser session.

Resource server (Nero APIs you call): `https://nero.grogan.dev`
Authorization server (register / claim / token): `https://nero.grogan.dev`

Nero APIs live on the resource host:
...
WWW-Authenticate: Bearer resource_metadata="https://nero.grogan.dev/.well-known/oauth-protected-resource"
```
...
```json
{
  "resource": "https://nero.grogan.dev/",
  "resource_name": "Nero",
  "authorization_servers": ["https://nero.grogan.dev/"],
  "scopes_supported": ["workspaces.read", "workspaces.write"],
...
- `resource` — the canonical URL of the API you're trying to call (`https://nero.grogan.dev/`). Use this as the `aud` when minting an ID-JAG.
...
```json
{
  ...
  "issuer": "https://nero.grogan.dev",
  "token_endpoint": "https://nero.grogan.dev/oauth2/token",
  "revocation_endpoint": "https://nero.grogan.dev/oauth2/revoke",
  ...
  "agent_auth": {
    "skill": "https://nero.grogan.dev/auth.md",
    "identity_endpoint": "https://nero.grogan.dev/agent/identity",
    "claim_endpoint": "https://nero.grogan.dev/agent/identity/claim",
    "events_endpoint": "https://nero.grogan.dev/agent/event/notify",
...
If anything in this file conflicts with the Protected Resource Metadata, treat the PRM as authoritative.
...
2. **You have only the user's email** → [service_auth](#service_auth). Claim ceremony required. The human completes the ceremony by signing in with AuthKit.
...
Before sending an `identity_assertion` or `service_auth` body, surface the service's `resource_name` (from Step 1a) and the scope set you'll be acting under, and confirm with the user. This is the user's only consent gate before their identity is asserted to the service. Skip this for `anonymous` — there is no user identity to assert.
...
- `aud` = the `resource` from the PRM (`https://nero.grogan.dev/`)
...
**Confirmation required (401)** — `(iss, sub)` is unknown but the ID-JAG's verified email or phone matched an existing account at the service. The service won't silently bind the delegation — the user has to confirm linking the provider identity to their account (they sign in with AuthKit).
...

{
  ...
  "registration_id": "reg_...",
  "registration_type": "identity_assertion",
  "claim_url": "https://nero.grogan.dev/agent/identity/claim",
  "claim_token": "clm_...",
  "claim_token_expires": "…",
  "post_claim_scopes": ["workspaces.read", "workspaces.write"],
  "claim": {
    "user_code": "123456",
    "expires_in": 600,
    "verification_uri": "https://nero.grogan.dev/auth/login?return_to=%2Fclaim%3Fclaim_attempt_token%3D...",
    "interval": 5
  }
}
```

Same `claim` block as the `service_auth` registration response — the user signs in to Nero via AuthKit, sees a confirmation page that names your provider, and types the `user_code` to confirm. Surface `verification_uri` + `user_code` to the user (see [Step 4b](#4b-hand-off-to-the-user)) and poll the token endpoint (see [Step 4c](#4c-poll-for-completion)).
...
The human signs in with AuthKit at `verification_uri`. Humans do not register via this document.
...
```json
{
  ...
  "identity_assertion": "<service-signed JWT>",
  "assertion_expires": "2026-05-04T13:00:00.000Z",
  "pre_claim_scopes": ["workspaces.read"],
  "claim_url": "https://nero.grogan.dev/agent/identity/claim",
  "claim_token": "clm_...",
...
The end goal: get a signed-in user to confirm a 6-digit `user_code` **you supply them**. The code travels from you → user, not from the service → user; the user authenticates to Nero with AuthKit and types the code into a page Nero owns. This binds the agent's context to the user's authenticated session. The ceremony fields (`user_code`, `verification_uri`, `expires_in`, `interval`) borrow their shape from [RFC 8628 device authorization](https://datatracker.ietf.org/doc/html/rfc8628), and polling happens at the standard `token_endpoint` with a profile-specific grant.
...
> Open this link, sign in with AuthKit (or sign up), and enter this 6-digit code: **123456**
> https://nero.grogan.dev/auth/login?return_to=...
...
2. Authenticate with AuthKit (existing user → sign in). Humans do not use agentic registration.
...
Use `access_token` immediately on Nero APIs; cache `identity_assertion` for the eventual refresh.
...
POST the `identity_assertion` to the AS metadata's `token_endpoint` with the [RFC 7523](https://datatracker.ietf.org/doc/html/rfc7523) JWT-bearer grant. The `resource` parameter is optional but recommended — it pins the access_token to the Nero API.
...
Present the `access_token` as a bearer token against Nero APIs on `https://nero.grogan.dev`:
...
Workspace daemon (same origin, path prefix):
...
Human browser clients keep using AuthKit (`wos-session` cookie). Do not mint an AuthKit cookie from this flow.
