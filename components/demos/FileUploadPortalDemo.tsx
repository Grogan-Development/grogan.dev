...
import { DemoButton, DemoSelect } from "./demoUi";
      ...
      {step === 0 && (
        <DemoPanel panelKey="upload">
          <DemoPlaceholder
            ...
            hint="PDF, AI, or EPS · max 250 MB"
            tall
          />
          <DemoSelect defaultValue="Vinyl banner">
      ...
      {step === 1 && (
        <DemoPanel panelKey="checklist">
          ...
          {(["format", "size", "bleed"] as const).map((key) => (
            <label
              key={key}
              className="flex min-h-9 items-center gap-2.5 border border-line bg-surface-alt/40 px-3 py-2 text-sm"
            >
              <input
                type="checkbox"
                checked={checks[key]}
